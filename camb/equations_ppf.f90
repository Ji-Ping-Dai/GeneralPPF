    ...
    real(dl) :: IDEcase = 1
    logical :: is_bc_dc_uc
    logical :: is_bc_dc_ude
    logical :: is_bde_dde_uc
    logical :: is_bde_dde_ude
    !ide_end
    ...
    if(.not. use_tabulated_w)then
        ...
        !ide_begin
        belta = Ini_Read_Double_File(Ini,'belta', -1.d0)
        IDEcase = Ini_Read_Double_File(Ini,'IDEcase', 1.d0)
        !ide_end
...
100     close(10)
    ...
    !ide_begin calculate rho_de in background
    if(.not. use_tabulated_w) then
        !grho_de=grhov*a**(1._dl-3.*w_lam-3.*wa_ppf)*exp(-3.*wa_ppf*(1._dl-a))
        if(a>0) then
            if (is_bc_dc_uc .or. is_bc_dc_ude) then
                grho_de=grhov*a**(-3._dl*w_lam+1._dl)*(1._dl+belta/(belta-w_lam)*grhoc/grhov*(1-a**(-3._dl*belta+3._dl*w_lam)))
            elseif(is_bde_dde_uc .or. is_bde_dde_ude) then
                grho_de=grhov*a**(-3._dl*(1._dl+w_lam-belta)+4._dl)
            endif
        else
            grho_de=0
        endif
...
1   if (khi-klo.gt.1) then
    ...
    is_cosmological_constant = w_lam==-1_dl .and. wa_ppf==0._dl
    !ide_begin
    is_bc_dc_uc = IDEcase == 1._dl
    is_bc_dc_ude = IDEcase == 2._dl
    is_bde_dde_uc = IDEcase == 3._dl
    is_bde_dde_ude = IDEcase == 4._dl
    !ide_end
    ...
    !  8*pi*G*rho*a**4.
    if (is_bc_dc_uc .or. is_bc_dc_ude) then
        rhoc=grhoc*a**(1._dl-3*belta)
    elseif(is_bde_dde_uc .or. is_bde_dde_ude) then
        rhoc=grhoc*a*(1._dl+belta/(belta-w_lam)*grhov/grhoc*(1._dl-a**(3._dl*belta-3._dl*w_lam)))
    endif

    !grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass
    grhoa2=grhok*a2+grhob*a+rhoc+grhog+grhornomass+grho_de(a)
    ...
    !ide_begin
    !two extra functions for Gamma and vc, also needed in Lcdm model 
    !Dark energy
    ...
    !ide_begin
    if (is_bc_dc_uc .or. is_bc_dc_ude) then
        grhoc_t=grhoc*a**(-1._dl-3*belta)
    elseif(is_bde_dde_uc .or. is_bde_dde_ude) then
        grhoc_t=grhoc/a*(1._dl+belta/(belta-w_lam)*grhov/grhoc*(1._dl-a**(3._dl*belta-3._dl*w_lam)))
    endif
    !ide_end
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        i_qg=5,i_qr=6,i_vb=7,i_pir=8, i_eta=9, i_aj3r=10,i_clxq=11,i_vq=12,i_vc=13
    ...
    real(dl) w_eff, grhoT
    !ide_begin
    real(dl) D1,D2,C1,C2,QIDE,ksi0,V0,Delta0,piG4,S,S1,S2,vc,vcdot,sigmaD
    !ide_end
    ...
    if(is_bc_dc_uc) then
        C1=0.0
        D1=0.0
        C2=3._dl*adotoa/a*belta
        D2=3._dl*adotoa*belta/a*grhoc_t/a2/piG4/2
        QIDE=3._dl*adotoa/a*belta*grhoc_t/a2/piG4/2
    elseif(is_bc_dc_ude) then 
        C1=0.0
        D1=3._dl*adotoa*belta/a*grhov_t/a2/piG4/2*(1+w_eff)
        C2=3._dl*adotoa/a*belta
        D2=0.0
        QIDE=3._dl*adotoa/a*belta*grhoc_t/a2/piG4/2
    elseif(is_bde_dde_uc) then
        C1=3._dl*adotoa/a*belta
        D1=0.0
        C2=0.0
        D2=3._dl*adotoa*belta/a*grhoc_t/a2/piG4/2
        QIDE=3._dl*adotoa/a*belta*grhov_t/a2/piG4/2
    elseif(is_bde_dde_ude) then
        C1=3._dl*adotoa/a*belta
        D1=3._dl*adotoa*belta/a*grhov_t/a2/piG4/2*(1+w_eff)
        C2=0.0
        D2=0.0
    ...
    Gamma=ay(EV%w_ix)
    ksi0=-((grhog_t*clxg+grhor_t*clxr)/3._dl-2._dl/9._dl*EV%kf(1)*(grhor_t*pir+grhog_t*pig)-a/k*D2*(vc-vT)*piG4*2._dl*a2)/(grhoT+gpres)
    V0=2._dl*k/Fa*Gamma*adotoa
    Delta0=-3._dl*adotoa/(piG4*a2*Fa)*adotoa*Gamma-k2*EV%kf(1)*Gamma/piG4/a2
    sigmaD = (etak + (dgrho + 3*adotoa/k*dgq)/2._dl/k)/EV%kf(1) - k*Gamma
    sigmaD = sigmaD/adotoa
    S_Gamma=grhov_t*(1+w_eff)*(vT+sigmaD)/2._dl/k
    ckH=c_Gamma_ppf*k/adotoa
    if (is_bc_dc_ude .or. is_bde_dde_ude) then
        S1=S_Gamma-3._dl*a*adotoa*D2/k/EV%kf(1)*(vc-vT)*piG4*a2/k2-C2*a/EV%kf(1)*grhoc_t*clxc/2._dl/k2-C2*a/EV%kf(1)/2._dl/k2*(3._dl*adotoa*grhoc_t+a*QIDE*2*piG4*a2)*vT/k&
        &-a/EV%kf(1)*piG4*a2/k2*QIDE*ksi0-piG4*a2/k2*(3._dl*a/EV%kf(1)*adotoa/k+a2/k/EV%kf(1)*QIDE*2._dl*piG4*a2/(grhoT+gpres))*3._dl*adotoa*belta/a/a2/piG4/2*V0&
        &-piG4*a2/k2*a/EV%kf(1)*C1*Delta0
        S2=-(a2/adotoa/EV%kf(1)*QIDE*2._dl*piG4*a2/(grhoT+gpres)+3._dl*a/EV%kf(1))*3._dl*adotoa*belta/a*adotoa/(Fa*k2)&
        &+a/EV%kf(1)*C1*3._dl*adotoa/Fa/k2
    elseif(is_bc_dc_uc .or. is_bde_dde_uc) then
        S1=S_Gamma-3._dl*a*adotoa*D2/k/EV%kf(1)*(vc-vT)*piG4*a2/k2-C2*a/EV%kf(1)*grhoc_t*clxc/2._dl/k2-C2*a/EV%kf(1)/2._dl/k2*(3._dl*adotoa*grhoc_t+a*QIDE*2*piG4*a2)*vT/k&
        &-a/EV%kf(1)*piG4*a2/k2*QIDE*ksi0-piG4*a2/k2*a/EV%kf(1)*C1*Delta0
        S2=a/EV%kf(1)*C1*3._dl*adotoa/Fa/k2
    endif
    Gammadot=(S1-(1._dl+ckH*ckH)**2*(S2+1._dl)*adotoa*gamma)/(ckH*ckH*(S2+1._dl)+1._dl)
    ...
    S=(S1+S2*Gammadot)/(S2+1._dl)
    dgqe=S-Gammadot-Gamma*adotoa
    dgqe=-dgqe/Fa*2._dl*k + vT*grhov_t*(1._dl+w_eff)
    dgrhoe=-2._dl*k2*EV%kf(1)*Gamma-3._dl/k*adotoa*dgqe
    dgrho=dgrho+dgrhoe
    dgq=dgq+dgqe
    ...
    clxcdot=a*QIDE*clxc/grhoc_t*2*piG4*a2-a*C1*grhov_t/grhoc_t*dgrhoe/grhov_t-a*C2*clxc-k*vc-k*z
    ayprime(3)=clxcdot
    if (is_bc_dc_ude .or. is_bde_dde_ude) then
        vcdot=(a*QIDE-a*D2)/grhoc_t*2*piG4*a2*vc-a/grhoc_t*2*piG4*a2*dgqe/grhov_t*3._dl*adotoa*belta/a*grhov_t/a2/piG4/2-adotoa*vc
    elseif(is_bc_dc_uc .or. is_bde_dde_uc) then
        vcdot=(a*QIDE-a*D2)/grhoc_t*2*piG4*a2*vc-adotoa*vc
    endif
